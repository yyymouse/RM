...
static void gimbal_feedback_update(gimbal *feedback_update)
{
    ...
    feedback_update->gimbal_pitch_motor.absolute_angle = pitch_motor_put_direction*(*(feedback_update->gimbal_INT_angle_point + INS_PITCH_ADDRESS_OFFSET));
...
static void gimbal_movement(gimbal *movement)   //控制云台角度
{
    ...
    movement->gimbal_pitch_motor.absolute_angle_set -= pitch_motor_put_direction*(movement->gimbal_rc->rc.ch[gimbal_pitch_CHANNEL]*pitch_rc_multiple -movement->gimbal_rc->mouse.y * PITCH_MOUSE_SEN);
...
//        }
    ...
    else if(loop->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_VISUAL)  //视觉模式
    {
        ...
        if(1/*(Visual_Ture.flag==1)&&( Visual_Ture.RInit_data[0] == 0x73) && (Visual_Ture.RInit_data[9] == 0x65 )*/)
        {
            ...
            if(fabs(loop->gimbal_pitch_motor.absolute_angle_set-loop->gimbal_pitch_motor.absolute_angle)<0.001)//自瞄消抖
            {
                loop->gimbal_pitch_motor.absolute_angle_set = loop->gimbal_pitch_motor.absolute_angle;

            }
            if(fabs(loop->gimbal_yaw_motor.absolute_angle_set-loop->gimbal_yaw_motor.absolute_angle)<0.001)
            {
        ...
        gimbal_movement1(&gimbal_ctrl);   //计算absolute_angle_set值
