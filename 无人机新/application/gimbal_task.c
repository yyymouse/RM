...
#include "DMPower.h"
#include "can.h"
extern dm4310_typedef_t *moto_4310;
int yaw_offset_ecd  =  4250;
static fp32 dm4310_p;
...
static void gimbal_feedback_update(gimbal *feedback_update)
{
    ...
    feedback_update->gimbal_pitch_motor.relative_angle = motor_ecd_to_angle_change(feedback_update->gimbal_pitch_motor.gimbal_motor_measure->ecd,
		...
		feedback_update->gimbal_pitch_motor.motor_gyro =  *(feedback_update->gimbal_INT_gyro_point + INS_GYRO_Y_ADDRESS_OFFSET);    //更新陀螺仪角速度
...
{
    return &gimbal_ctrl.gimbal_pitch_motor;
}
/**
  * @brief         云台初始化函数          
...
void gimbal_init(gimbal *gimbal_init)
{
  ...
  flash_date_init();
	...
	gimbal_init->gimbal_pitch_motor.pitch_angle = 0;
  ...
  gimbal_init->gimbal_rc = get_remote_control_point(); //获取遥控器值
	...
	gimbal_init->gimbal_INT_accel_point = get_accel_data_point();//获取加速度数据
  gimbal_init->gimbal_INT_mag_point = get_mag_data_point();
	 //gimbal_init->gimbal_INT_angle_point = get_angle_data_point();
  ...
  Enable_CtrlMotor(&hcan2,MOTOR2,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFC);
...
void set_to_zero(gimbal  *set_to_zero,fp32 set,fp32 v)
{
   if(set_to_zero->gimbal_INT_angle_point[1]>set)
	 {
	   while(set_to_zero->gimbal_INT_angle_point[1]>set)
		 {
        dm4310_p-=v;	 		 
			 MIT_CtrlMotor(&hcan2,MOTOR2,dm4310_p ,dm4310_v,dm4310_kp,dm4310_kd,dm4310_tff);
		 }
	 }
 if(set_to_zero->gimbal_INT_angle_point[1]<set)
	 {
  ...
  if(loop->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_ENCONDE)  //锁死角度，吊射模式
	{
		 ...
		 yaw_rc_multiple =   0.000001f;
     pitch_rc_multiple = 0.0000009f;
	...
	if(loop->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_GYRO	) //自稳模式,up
	{

		  HAL_GPIO_WritePin(GPIOC,GPIO_PIN_8,GPIO_PIN_SET);
		  yaw_rc_multiple = 0.000025f;
		  pitch_rc_multiple = 0.000015f;
         YAW_MOUSE_SEN = 0.0015f;
         PITCH_MOUSE_SEN = 0.002f;
		 gimbal_movement(&gimbal_ctrl);      //只计算云台遥感设置值
    ...
    static int16_t yaw_channel = 0, pitch_channel = 0;	
 			  ...
 			  loop->gimbal_pitch_motor.absolute_angle_set =  loop->add_pitch; //计算pitch轴目标角度值，传过来的值范围0.33到-0.6
    ...
    loop->gimbal_yaw_motor.current_set = PID_calc(&loop->gimbal_yaw_motor.gimbal_motor_gyro_pid, 
		...
		if(loop->gimbal_pitch_motor.absolute_angle_set>0.2)
		loop->gimbal_pitch_motor.absolute_angle_set = 0.2;
        
		if(loop->gimbal_pitch_motor.absolute_angle_set<-1.5)
		loop->gimbal_pitch_motor.absolute_angle_set = -1.5;
	...
	loop->gimbal_pitch_motor.current_set = PID_calc(&loop->gimbal_pitch_motor.gimbal_motor_gyro_pid, 
																			INS_angle[1], 
																		loop->gimbal_pitch_motor.motor_gyro_set);//双环控制 
	  ...
	  if(loop->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_ZERO )//如果云台关闭则清除所有输出值
			{
				...
				MIT_CtrlMotor(&hcan2,MOTOR2, 0, 0,0,0,0);
			}
    ...
    if(loop->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_GYRO	) //自稳模式,up
    {
        ...
        MIT_CtrlMotor(&hcan2,MOTOR2,loop->gimbal_pitch_motor.motor_gyro_set,0,dm4310_kp,dm4310_kd,dm4310_tff);
    }
    if(loop->gimbal_yaw_motor.gimbal_motor_mode == GIMBAL_MOTOR_ENCONDE	) //自稳模式,up
    {
         MIT_CtrlMotor(&hcan2,MOTOR2,loop->gimbal_pitch_motor.motor_gyro_set,0,dm4310_kp,dm4310_kd,dm4310_tff);
    }
    MIT_CtrlMotor(&hcan1,MOTOR2,loop->gimbal_pitch_motor.motor_gyro_set,0,dm4310_kp,dm4310_kd,dm4310_tff);
		osDelay(1);
    CAN_cmd_gimbal( -loop->gimbal_yaw_motor.current_set/**YAW_DIRECTION*/,0,0, 0);
