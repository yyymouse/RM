...
//	}
	...
	char * mode_str=cJSON_GetObjectItem(dat,"mode")->valuestring;
		usb_printf("%s\n",mode_str);
		cJSON* dat_visual = cJSON_GetObjectItem(dat,"dat_visual");
		cJSON* date = cJSON_GetObjectItem(dat_visual,"date");
		gimbal_ctrl.add_yaw		=cJSON_GetArrayItem(date,0)->valuedouble;//传入接收到的数据到yaw视觉参数
		gimbal_ctrl.add_pitch	=cJSON_GetArrayItem(date,1)->valuedouble;//传入接收到的数据到pitch视觉参数
		gimbal_ctrl.flag_shoot	= cJSON_GetArrayItem(date,2)->valuedouble;//传入接收到的数据到shoot参数
	...
	else if(strcmp(cJSON_GetObjectItem(dat,"mode")->valuestring,move_m)==0)
	{
	char * mode_str=cJSON_GetObjectItem(dat,"mode")->valuestring;
		usb_printf("%s\n",mode_str);
		cJSON* dat_nacigation = cJSON_GetObjectItem(dat,"dat_nacigation");
		cJSON* date = cJSON_GetObjectItem(dat_nacigation,"date");
		chassic_ctrl.move_x	=cJSON_GetArrayItem(date,0)->valuedouble;//传入接收到的数据到x运动
		chassic_ctrl.move_y	=cJSON_GetArrayItem(date,1)->valuedouble;//传入接收到的数据到y运动
		chassic_ctrl.spin_z = cJSON_GetArrayItem(date,2)->valuedouble;//传入接收到的数据到旋转z轴
