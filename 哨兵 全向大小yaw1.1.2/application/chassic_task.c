...
static void chassis_control_vector(chassis *chassis_move_to_vector)
{
    if(chassis_move_to_vector->Sport_mode==remote_mode)
    {
		...
		//底盘速度合成
        vx_set_channel = vx_channel *  CHASSIS_VX_RC_SEN + chassic_ctrl.move_x * x_multiply;
        vy_set_channel = vy_channel * -CHASSIS_VY_RC_SEN+ chassic_ctrl.move_y * y_multiply;
...
//	 gimbal_ctrl.gimbal_yaw_motor.absolute_angle_set= gimbal_ctrl.gimbal_yaw_motor.absolute_angle_set % pi;
    ...
    chassis_wheel_speed->wheel_speed[3] = -chassis_wheel_speed->set_vx + chassis_wheel_speed->set_vy + ( - 1.0f) * MOTOR_DISTANCE_TO_CENTER * chassis_wheel_speed->set_wz;
		//usb_printf("%f,%f,%f,%f,",chassis_wheel_speed->wheel_speed[0],chassis_wheel_speed->wheel_speed[1],chassis_wheel_speed->wheel_speed[2],chassis_wheel_speed->wheel_speed[3]);
...
static void chassic_mode_set(chassis *mode)
{
    ...
    else if(switch_is_mid(mode->chassis_RC->rc.s[s_right]))//右中反向小陀螺
    {
        mode->chassis_mode = chassis_mode_fan_spin;
    ...
    if(gimbal_ctrl.gimbal_rc->mouse.press_r == 1 || switch_is_up(mode->chassis_RC->rc.s[s_left]))  //左上或者鼠标右键开启视觉模式
    {
        gimbal_ctrl.gimbal_yaw_motor.gimbal_motor_mode = GIMBAL_MOTOR_VISUAL;
    }

    if( switch_is_mid(mode->chassis_RC->rc.s[s_left]))   //left_mid=gimbal_gyro  左中开启云台
    {
        gimbal_ctrl.gimbal_yaw_motor.gimbal_motor_mode = GIMBAL_MOTOR_GYRO;
        if(gimbal_ctrl.gimbal_rc->mouse.press_r == 1)//mouse_right=visual  左中且右键视觉
        {
            gimbal_ctrl.gimbal_yaw_motor.gimbal_motor_mode = GIMBAL_MOTOR_VISUAL;
        }	
...
static void chassic_mode_ctrl(chassis *mode_ctrl)
{
    ...
    if(mode_ctrl->chassis_mode == chassic_follow_chassic_yaw)   //底盘跟随云台
    {
		...
		//chassic_ctrl.set_wz = chassic_ctrl.spin_z * z_multiply;//导航旋转
    }
...
//				usb_printf("%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f\r\n",
//								mode_ctrl->chassis_motor[0].speed,//1
//								mode_ctrl->chassis_motor[1].speed,//2
//								mode_ctrl->chassis_motor[2].speed,//3
//								mode_ctrl->chassis_motor[3].speed,//4
//								mode_ctrl->wheel_speed[0],				//5
//								mode_ctrl->wheel_speed[1],				//6
//								mode_ctrl->wheel_speed[2],				//7
//								mode_ctrl->wheel_speed[3],				//8
//								mode_ctrl->set_vx,								//9
//								mode_ctrl->set_vy,								//10
//								mode_ctrl->set_wz,								//11
//								mode_ctrl->vx,										//12
